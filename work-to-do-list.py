...
def addWorkload():
    ...
    if workload:
        with open("work-workload.txt","a") as taskfile:
            taskfile.write(f"\n{workload}")
...
def deleteWorkload():
    ...
    if workload in workload_list:
        ...
        with open("work-workload.txt", 'w') as taskfile:
...
def openWorkloadFile():
    ...
    try:
        ...
        with open("work-workload.txt","r") as taskfile:               #With open function means it tells the computer to open and read the workload.txt file
...
title=Label(app, text="WORK TASK LIST",font="arial 20 bold",fg="black",bg="#CCFFAB")
